...
class SolarSystemAnimation(ThreeDScene):
    def construct(self):
        ...
        r_matrix = np.load(r"results analysis/Training-results-mesh-0.01-LiteV2/log1p(xtanh(x))/master_integration_800_years.npz")['r']
        
        raw_data = r_matrix.reshape(-1, 10, 3)
        ...
        left_title = Tex(r"\textbf{SLL-HNN Inner System}", font_size=32).scale(0.85).move_to(LEFT * 3.55 + UP * 3.3)
        right_title = Tex(r"\textbf{SLL-HNN Full System}", font_size=32).scale(0.85).move_to(RIGHT * 3.55 + UP * 3.3)
