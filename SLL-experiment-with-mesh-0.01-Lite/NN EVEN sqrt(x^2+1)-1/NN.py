...
# Configuration
name_iter = "HNN_EVEN_sqrt(x^2+1)-1"
dt_val = "dt_0.01"
name_ubi = "SLL-experiment-with-mesh-0.01-Lite/NN EVEN sqrt(x^2+1)-1"
...
def main():
    ...
    # Parameters
    # Train and test
    Sz = torch.tensor(np.load(f"Data/{dt_val}/normalization_vectors.npz")['Z_norm'].astype('float64'), dtype=float_user)
    Szd = torch.tensor(np.load(f"Data/{dt_val}/normalization_vectors.npz")['Z_dot_norm'].astype('float64'), dtype=float_user)
    z_train = torch.tensor(np.load(f"Data/{dt_val}/Z_train_opt.npz")['data'].astype('float64'), dtype=float_user)/Sz
    z_test = torch.tensor(np.load(f"Data/{dt_val}/Z_test_opt.npz")['data'].astype('float64'), dtype=float_user)/Sz
    z_train_dot = torch.tensor(np.load(f"Data/{dt_val}/Z_dot_train_opt.npz")['data'].astype('float64'), dtype=float_user)/Szd
    z_test_dot = torch.tensor(np.load(f"Data/{dt_val}/Z_dot_test_opt.npz")['data'].astype('float64'), dtype=float_user)/Szd
    ...
    class even(nn.Module):
        ...
        def forward(self, x):
            return torch.sqrt(x**2+1)-1
            
    H_net = fns.Block_HNN_SL(func_act_V=even)
    H_net.double()
