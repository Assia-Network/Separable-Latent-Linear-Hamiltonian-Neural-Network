...
* **Predicción (Autograd Simpléctico):** Extracción de gradientes simplécticos mediante diferenciación automática. El sistema deriva el Hamiltoniano respecto a los estados latentes ($\dot{{\vec{r}}}$<sub>lat</sub> $= \frac{dH}{d\vec{p}}$ y $\dot{{\vec{p}}}$<sub>lat</sub> $= -\frac{dH}{d\vec{r}}$).
